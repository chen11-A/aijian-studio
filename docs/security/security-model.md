...
Phase 0 桌面版不把“已获得同一 Windows 用户身份、可并发改写父目录重解析点、ACL 和进程环境的恶意进程”纳入应用层防护边界；这等同于本地账户完整性已失守。媒体导入仍会拒绝 UNC/映射盘，使用不跟随末段重解析点的稳定句柄、本地应用缓存和逐次哈希复核。Creator Beta 前还需通过低权限媒体进程、应用控制的导入根和安装器 ACL 收紧这一边界。
